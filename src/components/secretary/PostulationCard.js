...
const PostulationCard = (props) => {
  ...
  useEffect(async () => {
    setIsLoading(true);
    try {
      const res = await axios({ method: 'GET', url: `http://${process.env.REACT_APP_IP_HOST ? process.env.REACT_APP_IP_HOST : 'localhost'}:8082/api/v1/diplomates/${diplomadoId}/postulations/${postulationId}` });
  ...
  const editPostulationHandler = async (payload) => {
    ...
    try {
      ...
      await axios.put(`http://${process.env.REACT_APP_IP_HOST ? process.env.REACT_APP_IP_HOST : 'localhost'}:8082/api/v1/diplomates/${diplomadoId}/postulations/${payload.id}`, payload.postulation, { headers });
      console.log('enviado!');
      setIsLoading(false);
    } catch (error) {
      // eslint-disable-next-line
