...
const DiplomatePage = () => {
  ...
  useEffect(async () => {
    try {
      setIsLoading(true);
      const res = await axios({ method: 'GET', url: `http://${process.env.REACT_APP_IP_HOST}:8081/api/v1/diplomates/${diplomadoId}` });
  ...
  const getContactHandler = async (payload) => {
    ...
    try {
      ...
      await axios.post(`http://${process.env.REACT_APP_IP_HOST}:8081/api/v1/postulants`, payload, { headers });
      console.log('enviado!');
    } catch (error) {
      // eslint-disable-next-line
      console.log(error);
  ...
  const getInscriptionHandler = async (payload) => {
    ...
    try {
      // eslint-disable-next-line
      console.log("enviando...");
      await axios.post(`http://${process.env.REACT_APP_IP_HOST}:8082/api/v1/diplomates/${diplomadoId}/postulations`, payload, { headers });
      console.log('enviado!');
    } catch (error) {
      // eslint-disable-next-line
      console.log(error);
